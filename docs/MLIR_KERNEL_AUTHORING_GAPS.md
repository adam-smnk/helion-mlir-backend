...
- **New, separate gap found while testing Form 3**: a combined multi-dim tile whose load and store *reorder* the tile dimensions between source and destination (e.g. `out[m, tm, tp, :] = src[m, tp, tm, :]`, swapping `tm`/`tp`) is not supported — the loaded value's dimension order doesn't match what the store expects, and there is no transpose/permutation step in the lowering to reconcile them. This is a distinct capability (index-order transpose) from combined-tile support itself, and was out of scope here; needs an explicit `linalg.transpose` (or equivalent permuting insert) step in `memory_ops.py`/`control_flow.py`'s synthetic-store path.
- Combined multi-dim tile loops with an *external* loop-carried accumulator (e.g. a reduction over both combined dims at once) are explicitly rejected (`NodeLoweringError`) rather than silently mishandled — this combination wasn't exercised by any existing kernel and its semantics need dedicated design/testing before lowering it.
